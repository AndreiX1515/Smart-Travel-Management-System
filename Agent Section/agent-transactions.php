              ...
              <button class="nav-link active" id="pills-all-tab" data-bs-toggle="pill" data-bs-target="#pills-all" type="button" role="tab" aria-controls="pills-all" aria-selected="true">
                ...
                <span class="badge">
                  <?php
                    ...
                    {
                      $sql1 = "SELECT COUNT(*) AS totalBookings FROM BOOKING b 
                  ...
                  <?php
                    if ($agentRole != 'Head Agent') 
                    {
                      $sql1 = "SELECT COUNT(*) AS totalBookings FROM BOOKING b 
                              ...
                              LEFT JOIN client cl ON b.accountType = 'Client' AND b.accountId = cl.accountId
                              LEFT JOIN company cc ON cl.companyId = cc.companyId
                              WHERE b.accountId = $accountId AND (b.status = 'Reserved' OR b.status = 'Pending')";
                      $result = mysqli_query($conn, $sql1);
                    ...
                    else
                    {
                      $sql1 = "SELECT COUNT(*) AS totalBookings FROM BOOKING b 
                              ...
                              LEFT JOIN company cc ON cl.companyId = cc.companyId
                              WHERE b.agentCode = '$agentCode' 
                              AND (b.status = 'Reserved' OR b.status = 'Pending')
                              AND (COALESCE(co.companyId, '') = COALESCE('$companyId', '') 
                              OR COALESCE(cc.companyId, '') = COALESCE('$companyId', ''))";
                      $result = mysqli_query($conn, $sql1);
