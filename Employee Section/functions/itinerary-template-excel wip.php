...
// PhpSpreadsheet core classes
use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\IOFactory;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;
use PhpOffice\PhpSpreadsheet\Writer\Html;

// PhpSpreadsheet styling
use PhpOffice\PhpSpreadsheet\Style\Alignment;
use PhpOffice\PhpSpreadsheet\Style\Border;
use PhpOffice\PhpSpreadsheet\Style\Fill;

// Dompdf for PDF generation (used independently)
use Dompdf\Dompdf;
use Dompdf\Options;
...
if (isset($_POST['itineraryDetails']) && isset($_POST['daysDetails'])) {
    ...
    try {
        ...
        $pageMargins->setHeader(0.3);    // 0.3 inch header margin
        $pageMargins->setFooter(0.3);    // 0.3 inch footer margin
        ...
        setDay6($sheet, $dayData);
            ...
            // Clear any output buffer to avoid corrupted download
            if (ob_get_length()) {
                ob_end_clean();
            }
            ...
            $spreadsheet = generateItinerarySpreadsheet($itineraryDetails, $daysDetails); // Assume this function is implemented

            // Format: 'xlsx', 'pdf', or 'both'
            $format = $_POST['format'] ?? 'xlsx';

            // Sanitize filename
            $baseName = preg_replace('/[^a-zA-Z0-9_-]/', '_', $itineraryDetails['itineraryName'] ?? 'Itinerary');
            $xlsxFilename = $baseName . '.xlsx';
            $pdfFilename = $baseName . '.pdf';
            $zipFilename = $baseName . '_files.zip';
            ...
            if ($format === 'pdf') {
                // Convert to HTML using PhpSpreadsheet
            ...
            error_log('File generation failed: ' . $e->getMessage());
            http_response_code(500);
            echo "Error generating the file. Please contact support.";
        }
