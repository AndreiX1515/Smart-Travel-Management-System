...
// Define PDF class that extends TCPDF
class PDF extends TCPDF
{
    ...
    public function Header()
    {
        if ($this->getPage() == 1) { // Only show header on the first page
            ...
            $this->Cell(0, 3, $this->itineraryDetails['packageName'], 'LRT', 1, 'C', true);
            $this->SetFont('Helvetica', 'B', 10);
            $this->Cell(0, 3, 'KOREA TOUR ' . $this->itineraryDetails['noOfDays'] . ' DAYS & ' . ($this->itineraryDetails['noOfDays'] - 1) . ' NIGHTS', 'LRB', 1, 'C', true);
            ...
            $this->Cell(70, 8, date('d, M. Y', strtotime($this->itineraryDetails['periodStart'])) . ' - ' . date('d, M. Y', strtotime($this->itineraryDetails['periodEnd'])), 'B', 0, 'C');

            // Guide details
            $this->Cell(20, 8, 'GUIDE:', 'LB', 0, 'C');
            $this->Cell(60, 4, $this->itineraryDetails['guideName'], 'LRB', 1, 'C');

            // Contact details
            $this->SetXY(140, 51.25);
            $this->Cell(60, 4, $this->itineraryDetails['countryCode'] . ' ' . $this->itineraryDetails['contactNumber'], 'LRB', 1, 'C');
        }
    }
    ...
    // Method to add itinerary row dynamically
    private function addItineraryRow($day, $area, $itinerary, $mealPlan, $hotel, $yPosition, $itineraryHeight)
    {
        // Add the row for each day in the itinerary
        $this->SetXY(10, $yPosition);
        $this->Cell(13, $itineraryHeight, $day, 1, 0, 'C');
        $this->Cell(22, $itineraryHeight, $area, 1, 0, 'C');
        $this->Cell(125, $itineraryHeight, $itinerary, 1, 0, 'L');
        $this->Cell(30, $itineraryHeight, $mealPlan, 1, 1, 'C');
    }
  ...
  * For any unused portion is not refundable ",
            'LRTB',
            'L'
        );
  ...
  * Please be noted that below rate is with compulsory shop visiting, please kindly explain to pax that they
    ...
    Showcase (USD 30) / Red Pine Shop (USD 30) / Cosmetics Shop (USD 30) / DFS (USD 20)",
            'LRTB',
            'L'
        );
...
// Basic check for empty or malformed JSON
if (empty($itineraryDetails) || empty($daysDetails)) {
    die('Invalid data received for itinerary or days details.');
}

// Debug log
error_log('itineraryDetails: ' . print_r($_POST['itineraryDetails'], true));
error_log('daysDetails: ' . print_r($_POST['daysDetails'], true));
...
// Configure and start PDF
$pdf->SetMargins(10, 10, 10);
$pdf->AddPage();
$pdf->addHotelInfoTable();
$pdf->addItineraryHeader();

// Default height for itinerary items
$itineraryHeight = 50;
...
foreach ($daysDetails as $day) {
    $daysData = [[
        ...
        'itineraryHeight' => $itineraryHeight,
    ]];

    $methodName = 'day' . $day['day'];
    if (method_exists($pdf, $methodName)) {
        $pdf->$methodName($daysData);
    } else {
        $pdf->day($daysData); // Fallback generic handler
    }
}
...
// Output the PDF
$pdf->Output('Itinerary_' . $itineraryDetails['packageName'] . '.pdf', 'I');
