...
try {
    ...
    $sheet->setCellValue('C31', $guideName);
    ...
    if (!empty($voucher['guideMeeting'][0])) {
        ...
        $sheet->setCellValue('C37', strtoupper(date('F j, Y', strtotime($meeting['meetingDate'] ?? ''))));
        $sheet->setCellValue('E37', strtoupper(date('g:i A', strtotime($meeting['meetingTime'] ?? ''))));

        $meetingPlace = $meeting['meetingPlace'] ?? '';
        if (strtoupper(trim($meetingPlace)) === 'ICN') {
            $meetingPlace = 'Incheon Airport (Terminal 1)';
        }
        $sheet->setCellValue('G37', $meetingPlace);
    }


    // Includes Section
    if (!empty($voucher['includes']) && is_array($voucher['includes'])) {
        $row = 39; // Starting row for includes
    ...
    if (!empty($voucher['excludes']) && is_array($voucher['excludes'])) {
        $row = 43; // Starting row for excludes
