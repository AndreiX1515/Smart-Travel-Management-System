...
try {
    ...
    $sheet->setCellValue('C19', $guideName);
    ...
    if (!empty($voucher['guideMeeting'][0])) {
        ...
        $sheet->setCellValue('C25', strtoupper(date('F j, Y', strtotime($meeting['meetingDate'] ?? ''))));
        $sheet->setCellValue('E25', strtoupper(date('g:i A', strtotime($meeting['meetingTime'] ?? ''))));

        $meetingPlace = $meeting['meetingPlace'] ?? '';
        if (strtoupper(trim($meetingPlace)) === 'ICN') {
            $meetingPlace = 'Incheon Airport (Terminal 1)';
        }
        $sheet->setCellValue('G25', $meetingPlace);
    }


    // Includes Section
    if (!empty($voucher['includes']) && is_array($voucher['includes'])) {
        $row = 27; // Starting row for includes
    ...
    if (!empty($voucher['excludes']) && is_array($voucher['excludes'])) {
        $row = 31; // Starting row for excludes
