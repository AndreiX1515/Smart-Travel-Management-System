...
<div class="body-container">
  <div class="main-container">  
    <div class="content-header">
      ...
      <?php 
        // Assuming $agentId is already defined and contains the agent's ID
        $sql1 = "SELECT CONCAT(lName, ', ', fName, CASE 
                  WHEN mName IS NULL OR mName = 'N/A' THEN '' ELSE CONCAT(' ', LEFT(mName, 1), '.') END) AS agentName 
                FROM agent WHERE agentId = '$agentId'";

        // Execute the SQL query
        $result1 = $conn->query($sql1); // Replace $conn with your actual database connection variable

        // Check if the query returned any rows
        if ($result1 && $result1->num_rows > 0) 
        {
          $row = $result1->fetch_assoc(); // Fetch the result as an associative array
          $agentName = $row['agentName']; // Retrieve the agentName from the result
        }
        else 
        {
          $agentName = "N/A"; // Default value if no agent is found
        }
      ?>
      <h6 class="agent">Referred By: <?php echo htmlspecialchars($agentName, ENT_QUOTES, 'UTF-8'); ?> </h6>
    </div>
