        ...
        <div class="form-container">
            ...
            <form class="" id="registerForm" method="POST" action="register.php">
                ...
                <div class="bottom-register-div">
                    <button type="submit" class="btn" id="SubmitRegButton">Register</button>
	...
	$('#floatingEmail').on('blur', function () {
		...
		$.ajax({
				...
				success: function (response) {
					if (response.exists) {
							if (email.trim() === "") {
									// If the email field is empty, log an error and stop further actions
									console.error("Email field is empty. Please enter a valid email.");
									return; // Exit to prevent further execution
							} else {
									// If the email exists and the field is not empty, show the message
									$('#message-1').text('This email is already in use.').addClass('show');
									emailField.css('border', '1px solid lightcoral');
									showMessage(); // Call to handle fade out
							}
					} else {
							// If the email does not exist, reset the field's border
